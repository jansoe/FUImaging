...
void read_matrix_from_float(matrix* A, int rows, int cols, float* value){
    //create matrix with all elements from 'value'
...
void sum_cols_d(action_t action, matrix a, matrix c, int* params){
	...
	{
	    case 1024:
		reduce2D<1024><<< dimGrid, dimBlock, dimBlock.x*sizeof(float) >>>(a.mat_d,c.mat_d,N); break;
	    case 512:
	...
	{
		case 1024:
		reduce2D<1024><<< dimGrid, dimBlock, dimBlock.x*sizeof(float) >>>(a.mat_d,r1,N); break;
	    case 512:
	...
	{
		case 1024:
		reduce2D<1024><<< dimGrid2, dimBlock2, dimBlock2.x*sizeof(float) >>>(r1,c.mat_d,dimGrid.x); break;
	    case 512:
